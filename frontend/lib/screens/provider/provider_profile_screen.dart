...
class _ProviderProfileScreenState extends State<ProviderProfileScreen> {
  ...
  Future<void> _load() async =>
      Provider.of<ProviderProvider>(context, listen: false).getMyProviderProfile();
        ...
        child: CustomScrollView(slivers: [
            ...
            child: Padding(
              ...
              child: Container(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                  _Stat(value: p.rating.toStringAsFixed(1), label: 'Rating',  icon: Icons.star_rounded, color: AppTheme.accentColor),
                  _Vline(),
                  _Stat(value: '${p.totalReviews}', label: 'Reviews', icon: Icons.reviews_rounded, color: AppTheme.primaryColor),
                  _Vline(),
                  _Stat(value: '${p.totalBookings}', label: 'Jobs', icon: Icons.work_rounded, color: AppTheme.successColor),
                ]),
              ),
            ),
          ),
          ...
          SliverPadding(
            ...
            sliver: SliverList(delegate: SliverChildListDelegate([
              // Rate & Experience
              _card(children: [
                _Row(icon: Icons.currency_rupee_rounded, label: 'Hourly Rate',
                    value: '₹${p.hourlyRate.toStringAsFixed(0)} / hour',
                    color: AppTheme.accentColor),
                const Divider(height: 1, color: AppTheme.dividerColor, indent: 52),
                _Row(icon: Icons.timeline_rounded, label: 'Experience',
              ...
              _card(children: [
                _Row(icon: Icons.location_on_rounded, label: 'Service Radius',
                    value: '${p.serviceArea} miles',
                    color: AppTheme.successColor),
                if (user?.location?.fullAddress != null) ...[
                  const Divider(height: 1, color: AppTheme.dividerColor, indent: 52),
                  _Row(icon: Icons.home_rounded, label: 'Base Location',
              ...
              // Availability
              Row(children: [
                Expanded(child: _SectionLabel('Availability')),
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      const Icon(Icons.edit_rounded, size: 12, color: AppTheme.primaryColor),
                ...
                child: Container(
                  ...
                  child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    Text('Sign Out', style: GoogleFonts.inter(
                        color: AppTheme.errorColor, fontWeight: FontWeight.w700, fontSize: 15)),
                  ]),
                ),
              ),
  ...
  Widget _buildHeader(dynamic user, dynamic provider, ProviderProvider pp, AuthProvider auth) {
        ...
        child: Container(width: 180, height: 180,
            decoration: BoxDecoration(shape: BoxShape.circle,
                gradient: RadialGradient(colors: [
                  AppTheme.primaryColor.withAlpha(55), AppTheme.primaryColor.withAlpha(0)])))),
      SafeArea(child: Column(children: [
        Padding(
          padding: const EdgeInsets.fromLTRB(16, 8, 16, 0),
          child: Row(children: [
            Text('Provider Profile', style: GoogleFonts.inter(
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Icon(Icons.edit_rounded, size: 18, color: AppTheme.textPrimary),
              ),
            ),
          ]),
        ),
        ...
        Text(name, style: GoogleFonts.inter(fontSize: 20, fontWeight: FontWeight.w800, color: AppTheme.textPrimary)),
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: verified ? AppTheme.successColor.withAlpha(30) : AppTheme.warningColor.withAlpha(30),
            borderRadius: BorderRadius.circular(100),
            border: Border.all(
              color: verified ? AppTheme.successColor.withAlpha(80) : AppTheme.warningColor.withAlpha(80)),
          ),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(
            ...
            Text(
              verified ? 'Verified Provider' : 'Verification ${(provider.verificationStatus as String).toUpperCase()}',
              style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w700,
                  color: verified ? AppTheme.successColor : AppTheme.warningColor, letterSpacing: 0.5),
            ),
          ]),
        ),
    ...
    body: Center(child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
          width: 80, height: 80,
          decoration: BoxDecoration(
            color: AppTheme.navySurface, shape: BoxShape.circle,
            border: Border.all(color: AppTheme.dividerColor)),
          child: const Icon(Icons.work_outline_rounded, size: 40, color: AppTheme.textHint),
        ),
        const SizedBox(height: 20),
        Text('No Provider Profile', style: GoogleFonts.inter(
            fontSize: 20, fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
        const SizedBox(height: 8),
        ...
        Container(
          ...
          child: TextButton(
            ...
            child: Text('Create Profile', style: GoogleFonts.inter(
        ...
        TextButton.icon(
          ...
          label: Text('Sign Out', style: GoogleFonts.inter(color: AppTheme.errorColor, fontWeight: FontWeight.w600)),
        ),
      ]),
    )),
  );
    ...
    return List.generate(days.length, (i) {
      ...
      return Column(children: [
        Padding(
          ...
          child: Row(children: [
            Text(day, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600,
            ...
            Container(
              ...
              child: Text(
                avail.available ? '${avail.startTime} – ${avail.endTime}' : 'Unavailable',
                style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w700,
                    color: avail.available ? AppTheme.successColor : AppTheme.errorColor),
              ),
            ),
          ]),
        ),
        if (i < days.length - 1) const Divider(height: 1, color: AppTheme.dividerColor),
      ]);
    });
  }
  ...
  Widget _card({required List<Widget> children}) => Container(
    ...
    decoration: BoxDecoration(
      color: AppTheme.navySurface, borderRadius: BorderRadius.circular(20),
      border: Border.all(color: AppTheme.dividerColor)),
    ...
    final ok = await showDialog<bool>(context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(controller: ctrl, decoration: const InputDecoration(
            labelText: 'Image URL', hintText: 'https://example.com/photo.jpg')),
        actions: [
  ...
  Future<void> _showEditProviderDialog(BuildContext context, ProviderProvider pp) async {
    final p = pp.currentProvider;
    if (p == null) return;
    ...
    final skillCtrl = TextEditingController(text: (p.skills as List).join(', '));
    final ok = await showDialog<bool>(context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 12),
          TextField(controller: areaCtrl,  keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Service area (miles)')),
          const SizedBox(height: 12),
          TextField(controller: skillCtrl, maxLines: 2, decoration: const InputDecoration(labelText: 'Skills', hintText: 'Comma-separated')),
          const SizedBox(height: 12),
    ...
    if (ok != true || !context.mounted) return;
    final skills = (skillCtrl.text.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty)).toList();
  ...
  Future<void> _showEditAvailabilityDialog(BuildContext context, ProviderProvider pp) async {
    final p = pp.currentProvider;
    if (p == null) return;
    final editableDays = {
    ...
    final ok = await showDialog<bool>(context: context,
      builder: (ctx) => StatefulBuilder(builder: (_, setDs) => AlertDialog(
                ...
                child: Container(
                  ...
                  child: Column(children: [
                    Row(children: [
                      ...
                      Switch(value: avail.available,
                          onChanged: (v) => setDs(() => avail.available = v)),
                    ]),
                    if (avail.available) Row(children: [
                      Expanded(child: OutlinedButton(
                        onPressed: () async {
                          final t = await _pickTime(context, avail.startTime);
                          if (t != null) setDs(() => avail.startTime = t);
                        },
                      ...
                      Expanded(child: OutlinedButton(
                        onPressed: () async {
                          final t = await _pickTime(context, avail.endTime);
                          if (t != null) setDs(() => avail.endTime = t);
                        },
...
class EditableDayAvailability {
  ...
  EditableDayAvailability({required this.available, required this.startTime, required this.endTime});
  factory EditableDayAvailability.fromDay(DayAvailability d) =>
      EditableDayAvailability(available: d.available,
  ...
  DayAvailability toDayAvailability() =>
      DayAvailability(available: available, startTime: available ? startTime : '',
...
class _Vline extends StatelessWidget {
  @override
  Widget build(BuildContext context) => Container(width: 1, height: 40, color: AppTheme.dividerColor);
}
